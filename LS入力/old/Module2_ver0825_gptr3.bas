...
' 改訂: v3.1-fixed － Type ProcessResult を宣言部へ移動／2016互換
'  - 列は数値インデックスを使用（Cells(row, col)互換）
'  - UDTにObject/Worksheetを持たせない（禁則回避）
'  - XlCalculationはLongで保持
'  - DataObject失敗時はWinAPIフォールバック
...
' データ範囲(データ消去)
Private Const CLEAR_RANGE_ACQ   As String = "C8:F22"
Private Const CLEAR_RANGE_ACQ2  As String = "H8:H22"
...
Public Sub ClearInputData()
    ...
    wsAcq.Range("C4").ClearContents '[C4=任意日付]の消去
