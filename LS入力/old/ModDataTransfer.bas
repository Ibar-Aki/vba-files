...
Private Const ACCUMULATE_MODE As Boolean = False ' ★修正：上書きが基本動作のためFalseに変更（ただしコード内では未使用）
...
    ' 書き込み
    WriteAggregatedDataToSheet config, wsData, wsMonthly, aggregated, mapDict, lastCol, result
...
            If targetCol > 0 Then
                ' 引数から accumulateMode を削除し、wsData を追加
                WriteTimeDataToCell wsData, wsMonthly, config.targetRow, targetCol, aggregatedData(key), result
...
Private Sub LogDuplicateMessage( _
    ByRef wsData As Worksheet, _
    ByVal category As String, ByVal workNo As String, _
    ByVal oldValue As Double, ByVal newValue As Double)

    Dim message As String
    ' メッセージを「(上書)」に固定
    message = "既存値検出: [" & workNo & "|" & category & "] 旧=" & SerialToHHMMString(oldValue) & _
              " 新=" & SerialToHHMMString(newValue) & " (上書)"
    ' メッセージの書き込み先と引数を変更
    AppendMessageToCell wsData, message
End Sub
...
            message = message & "重複検知: " & result.DuplicateCount & " 件（黄色ハイライト表示）" & vbCrLf & _
                      "詳細はデータ登録シートのメッセージ欄を確認してください。" & vbCrLf
        End If
