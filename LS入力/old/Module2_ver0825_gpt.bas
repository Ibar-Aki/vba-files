...
' 改訂: v3.1 - 2016互換・完全コンパイル対応
'  - 列記号→数値インデックス化
'  - TypeにObject/Worksheetを持たせない（UDT禁則回避）
'  - XlCalculationをLongで保持
'  - Worksheet/Dictionaryは手続き内の局所変数で運用
'  - クリップボードはForms.DataObject失敗時にWinAPIへ自動フォールバック
' 対象: Excel 2016+ / Windows 10+ / 日本語環境
'===============================================================================
Option Explicit
...
Private Sub CopyTextToClipboardSafe(ByVal textToCopy As String)
    ...
    Set dataObject = CreateObject("Forms.DataObject") ' 参照設定不要／存在しない環境もある
...
Private Function ConvertToMinutesEx(ByVal timeValue As Variant) As Double
    ...
    If IsNumeric(timeValue) Then
        If InStr(1, CStr(timeValue), ".") > 0 Then
            ConvertToMinutesEx = CDbl(timeValue) * MINUTES_PER_DAY   ' シリアル想定
        Else
            ConvertToMinutesEx = ParseHHMMInteger(CLng(timeValue))   ' HHMM想定
        End If
        Exit Function
    End If
...
Public Sub ClearInputData()
    ...
    If MsgBox("『" & ACQUISITION_SHEET_NAME & "』『" & DATA_SHEET_NAME & "』の入力データをクリアします。" & vbCrLf & _
              "実行しますか？", _
              vbYesNo + vbQuestion + vbDefaultButton2, "データクリア確認") = vbNo Then
        GoTo CleanUp
    End If
    ...
    MsgBox "データクリアが完了しました。", vbInformation, "クリア完了"
...
ErrorHandler:
    MsgBox "データクリア処理でエラーが発生しました: " & Err.description, vbCritical, "クリア処理エラー"
    Resume CleanUp
End Sub
