...
| モジュール | 主な役割 |
| --- | --- |
| ModAppConfig | シート名や列番号の定数を集中管理 |
| ModCommonUtils | アプリ状態保存やシート保護のユーティリティ |
| ModGetOutlookSch | Outlookから予定を取得しシートへ出力 |
| ModDataClear | 取得・登録シートの入力値をクリア |
| ModDataTransfer | 日次データを集計して月次シートへ転記 |
| ModMonthlyMaintenance | 月次シートの全クリアとカレンダー更新 |
| ダブルクリック削除r1 | 行単位の入力値クリア（ダブルクリックイベント） |
