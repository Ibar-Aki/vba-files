...
' 作成者: Gemini (改良版)
' バージョン: 2.0r (区分＋作番対応、シート保護対応、API安定化)
'================================================================================
...
Sub TransferDataToMonthlySheet()
    ...
    For c = 3 To lastHeaderCol ' C列から開始
        Dim categoryName As String, workNoName As String, combinedKey As String
    ...
    If targetRow = 0 Then
        MsgBox "転記先シート「" & MONTHLY_SHEET_NAME & "」に、登録日「" & Format(targetDate, "m/d") & "」が見つかりませんでした。" & vbCrLf & _
               "B" & MONTHLY_DATA_START_ROW & ":B" & MONTHLY_DATA_END_ROW & " の範囲に日付が存在するか確認してください。", vbExclamation
        GoTo CleanUp
    End If
    ...
    For i = DATA_START_ROW To lastDataRow
        If Not (IsEmpty(wsData.Cells(i, "C").Value) And IsEmpty(wsData.Cells(i, "D").Value) And IsEmpty(wsData.Cells(i, "E").Value)) Then
            clipboardText = clipboardText & CStr(wsData.Cells(i, "C").Value) & vbTab & _
                                          CStr(wsData.Cells(i, "D").Value) & vbTab & _
                                          CStr(wsData.Cells(i, "E").Value) & vbCrLf
        End If
    Next i
...
Private Function ConvertToMinutes(ByVal timeValue As Variant) As Double
    ...
    If InStr(1, CStr(timeValue), ".") > 0 Then
        ConvertToMinutes = CDbl(timeValue) * 1440#
    Else
...
#End If
    ...
    If lpGlobalMemory <> 0 Then
        ...
        ' 0 の hwnd を渡して既定のクリップボードを開く
        If OpenClipboard(0) <> 0 Then
            Call EmptyClipboard
            Call SetClipboardData(CF_UNICODETEXT, hGlobalMemory)
            Call CloseClipboard
            ' SetClipboardData 成功後は OS 所有になるため GlobalFree は不要
        End If
    End If
End Sub
...
Private Function UnprotectSheet(ws As Worksheet) As Boolean
    ...
    Err.Clear
    Dim pw As String
    pw = InputBox("シート「" & ws.Name & "」は保護されています。" & vbCrLf & "パスワードを入力してください。", "保護の解除")
