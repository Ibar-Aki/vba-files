...
' --- 行・列番号定数（月次データシート）---
Private Const COL_MESSAGE  As Long = 1   ' A列：処理メッセージ記録列
Private Const COL_DATE     As Long = 2   ' B列：日付列
Private Const MONTHLY_WORKNO_ROW As Long = 8       ' 作番が記載されている行
Private Const MONTHLY_HEADER_ROW As Long = 9       ' 作業コードが記載されている行
Private Const MONTHLY_DATA_START_ROW As Long = 10  ' データ記録範囲の開始行
...
Private Sub ExecuteDataTransfer( _
    ...
    ' --- ステップ7：月次シートのメッセージ列ヘッダを確認・設定 ---
    EnsureMessageColumnHeader wsMonthly

    ' --- ステップ8：集計データを月次シートに書き込み ---
...
'===============================================================================
' 【機能名】重複メッセージの記録
' 【概要】  データが上書きされる際に、元の値をメッセージ列（A列）に記録します。
' 【引数】  wsMonthly: 「月次データ」シートオブジェクト
'           rowNum: 記録対象の行番号
'           category: 作業コード
'           workNo: 作番
'           oldValue: 上書きされる前の値（Excelシリアル値）
'===============================================================================
...
'===============================================================================
' 【機能名】メッセージ列ヘッダの確認と設定
' 【概要】  月次シートのメッセージ列（A列）にヘッダが設定されているか確認し、
'           空の場合は「メッセージ」というヘッダを設定します。
' 【引数】  wsMonthly: 月次データシートオブジェクト
'===============================================================================
...
'===============================================================================
' 【機能名】メッセージセルへの追記
' 【概要】  指定された行のメッセージセルに、新しいメッセージを追記します。
'           既存のメッセージがある場合は、改行で区切って後ろに追加します。
' 【引数】  wsMonthly: 月次データシートオブジェクト
'           rowNum: メッセージを追記する行番号
'           message: 追記するメッセージ文字列
'===============================================================================
Private Sub AppendMessageToCell(ByRef wsMonthly As Worksheet, ByVal rowNum As Long, ByVal message As String)
    With wsMonthly.Cells(rowNum, COL_MESSAGE)
        If Len(.value) = 0 Then
            ' --- セルが空の場合は、そのままメッセージを設定 ---
            .value = message
        Else
            ' --- 既存の値がある場合は、改行コードを挟んで追記 ---
...
'===============================================================================
' 【機能名】月次シートのエラー表示クリア
' 【概要】  処理開始時に、月次シートのエラーメッセージ表示セル（I1）をクリアします。
'===============================================================================
...
'===============================================================================
' 【機能名】月次シートへのエラー報告
' 【概要】  エラー発生時に、エラーメッセージを月次シートの指定セル（I1）に表示します。
...
Private Sub ShowTransferResults(ByRef result As ProcessResult)
    ...
    If result.Success Then
        ...
        If result.DuplicateCount > 0 Then
            message = message & "重複検知: " & result.DuplicateCount & " 件（詳細は黄色ハイライトとA列メッセージを確認）" & vbCrLf
        End If
